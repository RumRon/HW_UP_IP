...
nn_functions<-
  ...
  output <-
    ...
    aggregate(X = output$point_distance,by = list(output$thisPoint), FUN=mean)%>%
    mutate(point_distance = x)%>%
    arrange(as.numeric(Group.1))
...
measureFrom_Matrix <-
  as.matrix(st_c(mklb.sf))
measureTo_Matrix <-
  as.matrix(st_c(police.sf))
nn <-
  get.knnx(st_c(police.sf), st_c(mklb.sf), 2)$nn.dist
...
test2 <-  
  aggregate(output$point_distance,list(output$thisPoint), FUN=mean)%>%
  mutate(point_distance = x)%>%
  arrange(as.numeric(Group.1)) #%>%
...
test2 <- output %>%
  arrange(as.numeric(thisPoint))%>%
  summarize(output,pointDistance = mean(point_distance), groups = thisPoint)%>%
  #arrange(as.numeric(thisPoint)) %>%
  dplyr::select(-thisPoint) %>%
  pull()
...
#   mklb.sf %>%
#       mutate(
#         police_nn1 = nn_functions(st_c(mklb.sf), st_c(police.sf),2))
