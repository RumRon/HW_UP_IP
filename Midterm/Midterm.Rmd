...
mklb.sf<-
  mklb.sf %>%
  mutate(age=2022-yearbuilt,priceperft=price/heatedarea)%>%
  dplyr::select(commonpid,price,age,heatedarea,numfirepla,
                fullbaths,halfbaths,bedrooms,units,toPredict,priceperft)
...
```{r police_dis}
cmpd_police<-
  st_read("https://raw.githubusercontent.com/RumRon/MUSA_508/Midterm/CMPD_Police_Offices.geojson") %>%
  st_transform('EPSG:2264')
  ...
  mklb.sf %>% 
    mutate(
      police_nn1 = nn_function(st_coordinates(mklb.sf), 
                              st_coordinates(mklbpolice.sf), k = 1)) 
