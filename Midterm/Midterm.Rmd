...
```{r summary,    message = FALSE, warning = FALSE,  results = 'asis' }
#internal
...
In this part, we study the Summary Statistics of Model 1 to select variables to build our final model. We mainly considered p-values in determining the correlation of variables and removed the values with larger p-values. The R^2^ is 0.87, which is an is an okay fit.
...
```{r resultsmodel,   message = FALSE, warning = FALSE,  results = 'asis' }
...
```{r results,   message = FALSE, warning = FALSE, results = 'asis'}
...
The spatial lag of error plot shows that as home price errors increase, the nearby home price errors slightly increase, indicating that our model errors are rarely spatially autocorrelated.
</p>
...
```{r moranTest,   message = FALSE, warning = FALSE}
...
```{r delete,    message = FALSE, warning = FALSE, results = 'asis'}
