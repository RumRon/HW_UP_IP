...
inTrain <- caret::createDataPartition(
  y = paste(topredict_num$commonpid,topredict_num$price ),
  p = .60, list = FALSE)

# split data into training and test
model.training <- topredict_num[inTrain,]
model.test <- topredict_num[-inTrain,] 

## First model: All internal features, amenity features
model1 <- lm(price ~ ., data = st_drop_geometry(model.training) %>% 
...
```{r model2}
model2 <- lm(price ~ ., data = model.training %>% 
           dplyr::select(# internal features
                         price, prisqft, age, storyheigh,heatedarea,
...
reg.cv <- 
  train(price ~ ., data = st_drop_geometry(topredict_num) %>% 
          dplyr::select(colnames(factor_internal), 
                        colnames(factor_amenity),
                        colnames(factor_acs)),
...
```{r moranTest}
moranTest <- moran.mc(model.test$SalePrice.Error, 
