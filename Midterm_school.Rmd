...
Matrix_ACS <-st_join(Matrix_ACS1, Matrix_ACS2) %>%
  distinct(geometry, .keep_all = TRUE)

Matrix_ACS <-Matrix_ACS%>%filter(prisqft!= 'Inf')

Matrix_ACS <-
  select_if(st_drop_geometry(Matrix_ACS), is.numeric) %>% na.omit()

ggcorrplot(
  round(cor(Matrix_ACS), 1), 
  p.mat = cor_pmat(Matrix_ACS),
  colors = c('#05A167', "white", '#6897BB'),
  type="lower",
  insig = "blank") +  
    labs(title = "Correlation across numeric variables\n(Amenity)\n",
