...
```{r}
# Copy this chunk in the midterm
#internal
...
#internal_feature <-internal_feature%>%filter(prisqft!= 'Inf')

#internal_feature2 <-
#  select_if(st_drop_geometry(internal_feature), is.numeric) %>% na.omit() # check
# look numeric, factors, check variable ->
...
```{r correlation_matrix}
# Amenity
Matrix_Amenity <- mklb.sf%>%
  dplyr::select(price,prisqft,police_nn1, police_nn2, police_nn3, school_nn1, school_nn2, school_nn3, church_nn1, church_nn2, church_nn3, park_nn1, park_nn2, park_nn3,firestation_nn1, firestation_nn2, firestation_nn3, medical_nn1, medical_nn2, medical_nn3,police_nn1, police_nn2, police_nn3)

Matrix_Amenity <-Amenity%>%filter(prisqft!= 'Inf')

Matrix_Amenity <-
  select_if(st_drop_geometry(Amenity), is.numeric) %>% na.omit()

ggcorrplot(
  round(cor(Matrix_Amenity), 1), 
  p.mat = cor_pmat(Matrix_Amenity),
  colors = c('#05A167', "white", '#6897BB'),
  type="lower",
  insig = "blank") +  
    labs(title = "Correlation across numeric variables\n(Amenity)\n",
...
Matrix_ACS2 <- tracts20%>%
  dplyr::select(pctWhite, pctBachelors, pctPoverty, TotalPop, MedHHInc, Singleadult, Healthins, Familyincome, Housingunits, Househeatingfuel)

factor_acs <-st_join(Matrix_ACS1, Matrix_ACS2) 
...
ggcorrplot(
  round(cor(factor_acss), 1), 
  p.mat = cor_pmat(factor_acss),
  colors = c('#05A167', "white", '#6897BB'),
  type="lower",
  insig = "blank") +  
    labs(title = "Correlation across numeric variables\n(Amenity)\n",
...
## 2.5 Map Home Prices
The sale price per SqFt in Mecklenburg County shows a clear spatial pattern. The sale price in Miami Beach and Downtown Miami is significantly higher than other areas.

## 2.6 interesting maps
...
ggplot() +
  ...
  geom_sf(data = factor_acs.sf, aes(colour = pctPoverty),
          show.legend = "point", size = .75) +
  #scale_colour_manual(values = palette5,
                   #labels=qBr(maphomeprice,"prisqft"),
                  #name="Quintile\nBreaks") +
  labs(title="Percentage of poverty, Mecklenburg County") +
  mapTheme()
```
```{r}
# plot map of age
...
ggplot() +
  geom_sf(data = st_union(tracts20), fill = "grey50") +
  geom_sf(data = factor_internal.sf, aes(colour = age),
          show.legend = "point", size = .75) +
  #scale_colour_manual(values = palette5,
                   #labels=qBr(maphomeprice,"prisqft"),
                   #name="Quintile\nBreaks") +
  labs(title="Housing age in Mecklenburg County") +
  mapTheme()
```
...
ggplot() +
  ...
  #scale_colour_manual(values = palette5,
                   #labels=qBr(maphomeprice,"prisqft"),
                   #name="Quintile\nBreaks") +
  labs(title="Distribution of average distance to nearest park in Mecklenburg County") +
  mapTheme()
```
