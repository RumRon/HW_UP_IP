...
final_net <-
  st_centroid(final_net) %>%
    st_join(dplyr::select(neighborhoods, district)) %>%  # if touch, add name
...
reg.ss.spatialCV <- crossValidate(
  ...
  indVariables = reg.ss.vars) %>%
    dplyr::select(cvID = district, countCrimes, Prediction, geometry)
```
...
as.data.frame(crime_KD.1000) %>%
  ...
  aggregate(., final_net, mean) %>%
   ggplot() +
     ...
     geom_sf(data = sample_n(crimes, 1000), size = .5) +
     scale_fill_viridis(name = "Density") +
...
correlation.long <-
  st_drop_geometry(final_net) %>%
    dplyr::select(-uniqueID, -cvID, -district, -district.x, -district.y, -districts) %>%
