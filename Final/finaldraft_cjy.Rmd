...
reg.ss.spatialCV <- crossValidate(
  ...
  indVariables = reg.ss.vars) %>%
    dplyr::select(cvID = name, countCrimes, Prediction, geometry)
```
From the distribution of MAE, we can find that the Spatial LOGO-CV works better because its MAE is more clustered and with smaller numbers.
...
## 3.4 MAE by race context and comparison for a random k-fold vs. spatial cross validation regression.

### 3.4.1 A table of raw errors by race context
Race is an important factor to predict the heroin overdose. So, race can be used to test our model. As the table shows, race don't have much effect on our model, but the model works better for white districts. The similarity for the race results of two models might because in this case we used the council districts of Mesa, which is too big for spacial LOGO-CV.Plus, Mesa is a predominantly white city with little racial influence.
...
### 3.4.2 A table of raw errors comparison
Similarly, there are too few areas to see a clear difference from the graph. But the spacial process is still an important part in our model.
...
The risk prediction model narrowly edges out the Kernel Density in the highest risk categories - suggesting this simple model has some value relative to the business-as-usual hot spot approach. So to some extent, the model is better than the kernal Density prediction model.
...
**Observation bias**  heroin overdose are often unobserved because somebody carrying or using heroin is frequently not arrested or sent to hospital - we only observe the event when there is an incident or a police presence.
