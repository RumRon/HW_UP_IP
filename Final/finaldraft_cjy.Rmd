...
In the feature selection part, all the features were divided into three parts.Risk Factors and Amenities.
...
do.call(grid.arrange,c(mapList, ncol=4,nrow=2, top="Risk Factors by Fishnet"))
```
...
```{r}
select_factors <-vars_net%>%dplyr::select(-Rail_station.nn)
vars_net_nn2 <- 
  dplyr::select(select_factors, ends_with(".nn")) %>%
    gather(Variable, value, -geometry)

ggplot() +
      geom_sf(data = vars_net_nn2, aes(fill=value), colour=NA) +
...
reg.ss.vars <- c("Un_Sheltered.nn","Trans_graffiti.nn","crime.isSig.dist", "Plc_incidence.nn", "Plc_dispatch.nn", "Code_violation.nn", "Park.nn", "Light.nn", "Rail_station.nn")
## RUN REGRESSIONS
...
crime_KD.df <- rbind(
  mutate(data.frame(rasterToPoints(mask(raster(crime_KD.1000), as(CouncilDistricts, 'Spatial')))), Legend = "1000 Ft."),
  mutate(data.frame(rasterToPoints(mask(raster(crime_KD.1500), as(CouncilDistricts, 'Spatial')))), Legend = "1500 Ft."),
  mutate(data.frame(rasterToPoints(mask(raster(crime_KD.2000), as(CouncilDistricts, 'Spatial')))), Legend = "2000 Ft.")) 
...
reg.vars <- c("Un_Sheltered.nn","Trans_graffiti.nn","crime.isSig.dist", "Plc_incidence.nn", "Plc_dispatch.nn", "Code_violation.nn", "Park.nn", "Light.nn", "Rail_station.nn")
...
oid being a tool of the surveillance state.
>>>>>>> 89c18ed355b0c53e8df7f27f1d68e8995b528f6f
