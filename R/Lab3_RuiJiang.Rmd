---
title: "MUSA 508 Lab 3 - Lab Exercise"
author: "Rui Jiang"
date: "September, 2022"
output:
  html_document:
...
census_api_key("YOUR API KEY", overwrite = TRUE, install= TRUE)
```
...
```{r first_maps}
ggplot()+
  geom_sf(data = studyAreaTowns, aes(fill = MUNI))+ #use MUNI to give color
...
```{r ownTheme}
function(base_size = 20, title_size = 16) {
  theme(
...
```{r mapping_challenge1}


ggplot() + 
  geom_sf(data = studyAreaTowns, aes(colour = MUNI))+
  geom_sf(data= buffersAndTowns, aes(fill = Legend))+
  scale_fill_manual(values = c("#F8766D", "#00BFC4")) + 
  labs(title = "Lancaster Municipalities within & ouside 1/8 mile UGA boundary") +
  mapTheme()
...
ggplot() + 
  ...
  geom_sf(data = filter(buffersAndTowns , Legend == "Outside"), aes(fill = MUNI), color = "transparent")+
