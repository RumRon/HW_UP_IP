...
advertising <- read.csv("https://raw.githubusercontent.com/urbanSpatial/Public-Policy-Analytics-Landing/master/DATA/Chapter6/housingSubsidy.csv")
...
```{r country_variables, eval=FALSE, cache=FALSE, include=FALSE}
###
advertising <- 
  advertising %>% 
  group_by(Country) %>% 
...
trainIndex <- createDataPartition(advertising$y, p = .50,
                                  list = FALSE,
                                  times = 1)
...
advertisingModel <- glm(y_numeric ~ .,
...
```{r second_model, eval=FALSE, include=FALSE}
...
testProbs <- data.frame(Outcome = as.factor(advertisingTest$y_numeric),
...
cost_benefit_table <-
   testProbs %>%
      ...
      summarize(True_Negative = sum(n[predOutcome==0 & Outcome==0]),
       ...
       mutate(Revenue =
               ifelse(Variable == "True_Negative", Count * 12400 ,
               ifelse(Variable == "True_Positive",0,
               ifelse(Variable == "False_Negative", -2850 * Count,
               ifelse(Variable == "False_Positive", 0, 0))))) %>%
...
iterateThresholds <- function(data) {
  ...
  this_prediction <-
     ...
     gather(Variable, Count) %>%
     mutate(Revenue =
               ifelse(Variable == "True_Negative", 12400 * Count,
               ifelse(Variable == "True_Positive",0,
               ifelse(Variable == "False_Negative", -2850 * Count,
               ifelse(Variable == "False_Positive", 0, 0)))),
            Threshold = x)
