...
dat <- dat %>%
  ...
  reanme(., from_station_name = start_station)
```
...
Notice we use the time format `ymd_hms` to denote year, month, day and hour, minute and second. We extract the `week` of the observation (ranging from 1-52 throughout the year) and the `dotw` for day of the week.
...
dat_census <- st_join(dat2 %>% 
          ...
          st_as_sf(., coords = c("from_longitude", "from_latitude"), crs = 2272),
        chicagoTracts %>%
  ...
  as.data.frame() %>%
  select(-geometry)%>%
  st_as_sf(., coords = c("to_longitude", "to_latitude"), crs = 2272) %>%
...
weather.Panel <- 
  riem_measures(station = "PHL", date_start = "2022-04-25", date_end = "2022-05-09") %>%
